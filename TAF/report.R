...
xyplot(f~year|area, groups=stage, f.stage, type=c("l","g"), as.table=TRUE,
...
xyplot(f~age|area, f.last.10, as.table=TRUE, type="l", lwd=2, xlab="Year",
       ylab="Fishing mortality")
dev.off()
taf.png("f_last_10_free_axes", width=2000, height=1600, res=250)
f.last.10 <- f.annual[f.annual$year %in% tail(sort(unique(f.annual$year)), 10),]
f.last.10 <- aggregate(f~age+area, f.last.10, mean)
xyplot(f~age|area, f.last.10, as.table=TRUE, type="l", lwd=2, scale="free",
